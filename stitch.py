...
def pre_allocate_arrays(channels, input_height, input_width, organized_data):
    max_i = max_j = max_z = 0
    ...
    stitched_images = np.zeros(tczyx_shape, dtype=np.uint16)
    return stitched_images

def pre_allocate_canvas(channels, input_height, input_width, organized_data, vertical_shift, horizontal_shift):
    max_i = max_j = max_z = 0
    for channel_data in organized_data.values():
        for z_data in channel_data.values():
            for tile_info in z_data:
    ...
    for channel, channel_data in organized_data.items():
        ...
        for z_level, z_data in channel_data.items():
            ...
            for tile_info in z_data:
                ...
                stitched_images[0, channel_idx, z_level, y:y+input_height, x:x + input_width] = tile.astype('uint16')
    ...
    for channel, channel_data in organized_data.items():
        ...
        for z_level, z_data in channel_data.items():
            ...
            for tile_info in z_data:
                ...
                print(f"   Placing Tile: C:{channel_idx},\tZ:{z_level},\tY:{y}-{y+tile.shape[-2]},\tX:{x}-{x+tile.shape[-1]}")
