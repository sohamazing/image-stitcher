...
from stitcher_hcs import Stitcher
...
class StitchingThread(QThread):
    ...
    def run(self):
        try:
            ...
            for well in self.stitcher.wells:
                # calculate translations to align images
                self.start_stitching.emit()
                if self.use_registration: 
                    self.stitcher.calculate_shifts(well, self.registration_z_level, self.registration_channel, self.v_max_overlap, self.h_max_overlap)

                # stitch images onto allocated dask array
                self.stitcher.stitch_images_cropped(well, progress_callback=self.update_progress.emit)
