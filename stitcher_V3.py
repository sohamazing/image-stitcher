...
class Stitcher(QThread, QObject):
    ...
    def __init__(self, input_folder, output_name='', output_format=".ome.zarr", apply_flatfield=0, use_registration=0, registration_channel='', registration_z_level=0, overlap_percent=0, flexible=True):
...
class CoordinateStitcher(QThread, QObject):
    ...
    def parse_filenames_xyz(self):
        ...
        max_z = 0
        fov_count = {}  # Dictionary to keep track of FOV count per region
        ...
        for t, time_point in enumerate(self.time_points):
            ...
            for file in image_files:
                ...
                channel = ' '.join(parts[4:]).split('.')[0]  # Join the rest for channel name and remove extension

                # Find the corresponding row in coordinates_df
                coord_row = coordinates_df[(coordinates_df['region'] == region) & 
                                           (coordinates_df['x (mm)'].round(3) == round(x, 3)) & 
                                           (coordinates_df['y (mm)'].round(3) == round(y, 3)) & 
                                           (coordinates_df['z_level'] == z)]
                if coord_row.empty:
                    print(f"Warning: No matching coordinates found for file {file}")
                    continue
                coord_row = coord_row.iloc[0]

                # Use (x, y) as a unique identifier for each FOV
                fov_id = (x, y)
                if region not in fov_count:
                    fov_count[region] = set()
                fov_count[region].add(fov_id)

                key = (t, region, fov_id, z, channel)
        ...
        self.num_z = max_z + 1
        self.num_fovs_per_region = max(len(fovs) for fovs in fov_count.values())
        ...
        self.mono_channel_names = []
        for channel in self.channel_names:
            channel_key = (t, first_key[1], first_key[2], first_key[3], channel)
    ...
    def run(self):
        ...
        if self.normal_naming:
            self.parse_filenames()
        else:
            self.parse_filenames_xyz()
